...
yval=np.convolve(np.mean(bp,0),box,mode='same')
plt.plot(xval,yval)
plt.xlabel('neuron (°)')
...
beh['pv']=beh.popvec.apply(beh.eval).values/np.pi*180
mnbystim=beh[['stimat','pv']].groupby('stimat').agg(count=('stimat', lambda x: len(x)), meanpv=('pv', lambda x: np.vstack(x).mean(axis=0).tolist()))
beh=beh.merge(mnbystim, on='stimat')
beh['pv_diff'] = beh['pv'] - beh['meanpv']
pv_diff = np.array(beh.pv_diff.tolist())
varpv = np.var(pv_diff,axis=0)
xval = np.arange(varpv.shape[0])*0.1-3
plt.plot(xval, varpv)
plt.ylim([0,0.6])
plt.xlim([-0.5, 3])
